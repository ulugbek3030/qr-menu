  ...
  app.innerHTML = `
    <div class="px-5 animate-[fadeIn_0.25s_ease-out]">
      ...
      <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="font-headline text-2xl font-extrabold tracking-tighter uppercase">${cat ? cat.name : 'Menu'}</h1>
      </div>

      <!-- Filters -->
      <div class="flex gap-2 overflow-x-auto pb-4 no-scrollbar -mx-5 px-5 mb-6">
        ${FILTERS.map(f => `
          <button data-filter="${f.id}" class="filter-btn whitespace-nowrap px-5 py-2 rounded-full font-medium text-sm transition-all ${activeFilter === f.id ? 'bg-primary text-on-primary shadow-lg shadow-primary/10' : 'bg-surface-container-highest text-on-surface-variant hover:text-primary'}">${f.label}</button>
        `).join('')}
      </div>
...
function renderDishes(allDishes, filter) {
  ...
  if (!dishes.length) {
    return `<p class="text-center text-on-surface-variant py-12 text-sm">No dishes in this category yet.</p>`;
  }
  ...
  return `
    ...
    <a href="#/dish/${hero.id}" class="block relative group overflow-hidden rounded-lg bg-surface-container-low">
      <div class="aspect-[16/9] w-full relative">
        <img class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" src="${hero.image}" alt="${hero.name}" loading="lazy">
        ...
        <div class="absolute bottom-0 left-0 p-5 w-full">
          ...
          <h2 class="font-headline text-2xl font-extrabold tracking-tighter text-on-surface uppercase">${hero.shortName}</h2>
          <p class="font-body text-on-surface-variant text-xs mt-1 line-clamp-2">${hero.description}</p>
          <div class="flex items-center justify-between mt-3">
            <span class="text-xl font-headline font-bold text-primary">${formatPrice(hero.price)}</span>
            <button data-add="${hero.id}" class="add-btn px-6 py-2.5 rounded-full gold-gradient text-on-primary font-bold tracking-widest text-[10px] uppercase active:scale-95 transition-all">Add to Order</button>
          </div>
        </div>
      </div>
    </a>
    ...
    ${rest.map(dish => `
      <div class="bg-surface-container-low rounded-lg overflow-hidden group">
        <a href="#/dish/${dish.id}" class="block aspect-square relative overflow-hidden">
          <img class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" src="${dish.image}" alt="${dish.name}" loading="lazy">
        ...
        <div class="p-5">
          <div class="flex justify-between items-start mb-1.5">
            <h3 class="font-headline text-lg font-bold tracking-tight text-on-surface uppercase">${dish.shortName}</h3>
            <span class="text-primary font-bold text-sm">${formatPrice(dish.price)}</span>
          </div>
          <p class="font-body text-on-surface-variant text-xs mb-4 line-clamp-2">${dish.description}</p>
          <button data-add="${dish.id}" class="add-btn w-full py-2.5 rounded-full border border-outline-variant text-on-surface text-[10px] font-bold tracking-widest uppercase hover:bg-primary hover:text-on-primary hover:border-primary transition-all">Add to Order</button>
        </div>
      </div>
    `).join('')}
  `;
